...
import sttp.tapir.{auth, stringBody, EndpointInput}
...
object SetSessionTapir extends App with StrictLogging {
  ...
  implicit def f: UsernamePassword => Option[MyScalaSession] = up =>
    Some(MyScalaSession(up.username))
  ...
  println(
    "Server started, press enter to stop. Visit http://localhost:8080/docs to see the swagger documentation."
  )
  StdIn.readLine()
