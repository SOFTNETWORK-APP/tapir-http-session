...
import com.softwaremill.session.{Legacy, MultipleTransportTest, SessionManager, TestData}
...
class OneOffTapirTest
  ...
  )(implicit manager: SMan): ServerEndpoint[Any, Future] =
  ...
  List(TestUsingCookies, TestUsingHeaders).foreach { using =>
    ...
    p should "migrate v0.5.1 session with migration config" in {
      Get("/set") ~> routes ~> check {
        ...
        Get("/getReq") ~> addHeader(using.setSessionHeader(legacySession)) ~> routes(
          manager_tokenMigrationFromV0_5_1
        ) ~> check {
    ...
    p should "migrate v0.5.2 session with migration config" in {
      Get("/set") ~> routes ~> check {
        ...
        Get("/getReq") ~> addHeader(using.setSessionHeader(legacySession)) ~> routes(
          manager_tokenMigrationFromV0_5_2
        ) ~> check {
          using.getSession should be('defined)
          responseAs[String] should be(data.toString)
        }
