...
import com.softwaremill.session.{SessionManager, TestData}
...
class CsrfEndpointsTest extends AnyFlatSpec with ScalatestRouteTest with Matchers {
  ...
  def transferMoneyEndpoint[T](implicit
    manager: SessionManager[T],
    checkMode: TapirCsrfCheckMode[T]
  ): ServerEndpoint[Any, Future] = {
  ...
  it should "accept requests if the csrf cookie matches the form field value" in {
    ...
    Get("/site") ~> testRoutes ~> check {
      ...
      Post(
        "/transfer_money",
        FormData(sessionConfig.csrfSubmittedName -> csrfCookie.cookie.value)
      ) ~>
      addHeader(Cookie(cookieName, csrfCookie.cookie.value)) ~>
      testRoutes ~>
      check {
